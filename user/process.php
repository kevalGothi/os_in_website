...
if ($fetch) {
    $number = htmlspecialchars($fetch["number"]); // Sanitize the phone number
} else {
    $number = ''; // Default value if no record is found
}
?>

<label class="col-sm-2 col-form-label" style="color:#000;">Phone Number</label>
<div class="col-sm-10">
    <input type="tel" placeholder="<?php echo $number; ?>" value="<?php echo $number; ?>" id="" name="phone" class="form-control autonumber" required>
</div>
<?php
    } else {
